...
from Functions import generate_grid_unit, save_img, save_flow, transform_unit_flow_to_flow, load_4D_with_header, imgnorm
...
parser.add_argument("--modelpath", type=str,
                    dest="modelpath", default='../Model/LDR_NLST_NCC_unit_disp_add_fea4_reg01_10_lite_stagelvl3_60000.pth',
...
def test():
    ...
    with torch.no_grad():
        ...
        save_flow(F_X_Y_cpu, savepath + '/warpped_flow_full_' + 'reg' + str(reg_input) + '.nii.gz', header=header, affine=affine)
        save_img(X_Y, savepath + '/warpped_moving_full_' + 'reg' + str(reg_input) + '.nii.gz', header=header, affine=affine)
